...
static int scoreForDestroyedMeteors = 0;
...
void InitGame(void)
{
    ...
    player.position = Vector2{ static_cast<float>(screenWidth) / 2, screenHeight / 2 - shipHeight / 2 };
    ...
    for (int i = 0; i < MAX_BIG_METEORS; i++)
    {
        ...
        bigMeteor[i].position = Vector2{ static_cast<float>(posx), static_cast<float>(posy) };
        ...
        bigMeteor[i].speed = Vector2{ static_cast<float>(velx), static_cast<float>(vely) };
                        ...
                        if (bigMeteor[a].active && CheckCollisionCircles(shoot[i].position, shoot[i].radius, bigMeteor[a].position, bigMeteor[a].radius))
                        {
                            ...
                            for (int j = 0; j < 2; j++)
                            {
                                if (midMeteorsCount % 2 == 0)
                                {
                                    ...
                                    mediumMeteor[midMeteorsCount].speed = Vector2{ static_cast<float>(cos(shoot[i].rotation * DEG2RAD) * METEORS_SPEED * -1), static_cast<float>(sin(shoot[i].rotation * DEG2RAD) * METEORS_SPEED * -1) };
                                }
                                else
                                {
                                    mediumMeteor[midMeteorsCount].position = Vector2{ bigMeteor[a].position.x, bigMeteor[a].position.y };
                                    mediumMeteor[midMeteorsCount].speed = Vector2{ static_cast<float>(cos(shoot[i].rotation * DEG2RAD) * METEORS_SPEED), static_cast<float>(sin(shoot[i].rotation * DEG2RAD) * METEORS_SPEED) };
                                }
                        ...
                        if (mediumMeteor[b].active && CheckCollisionCircles(shoot[i].position, shoot[i].radius, mediumMeteor[b].position, mediumMeteor[b].radius))
                        {
                            ...
                            for (int j = 0; j < 2; j++)
                            {
                                if (smallMeteorsCount % 2 == 0)
                                {
                                    ...
                                    smallMeteor[smallMeteorsCount].speed = Vector2{ static_cast<float>(cos(shoot[i].rotation * DEG2RAD) * METEORS_SPEED * -1), static_cast<float>(sin(shoot[i].rotation * DEG2RAD) * METEORS_SPEED * -1) };
                                }
                                else
                                {
                                    smallMeteor[smallMeteorsCount].position = Vector2{ mediumMeteor[b].position.x, mediumMeteor[b].position.y };
                                    smallMeteor[smallMeteorsCount].speed = Vector2{ static_cast<float>(cos(shoot[i].rotation * DEG2RAD) * METEORS_SPEED), static_cast<float>(sin(shoot[i].rotation * DEG2RAD) * METEORS_SPEED) };
                                }
...
void DrawGame(void)
{
    ...
    if (!gameOver)
    {
        ...
        const char* endScore = TextFormat("Score: %08i", scoreForDestroyedMeteors);
        DrawText(endScore, screenWidth / 1.5 - MeasureText(endScore, 20 / 2), screenHeight / 1.5 - 50, 20, RED);
